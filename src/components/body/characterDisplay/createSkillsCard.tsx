...
export function createSkillsCard(
    skills:
        {
            count: number,
            list: {
                name: string, score: number, stat: string, url: string
            }[]
        },
    stats: {
        unused: number;
        list: { index: string, name: string, score: number }[],
    ...
    let jsx = skills.list.map((skill: { stat: string, name: string, score: number, url: string }) => {
        if (stats && stats.list.length > 0) {
            // let statValue = Math.floor((stats.list.find( (stat:any) => stat.name === skills.stat).score - 10) / 2);
            let statValue = stats.list.find(({ index }) => index === skill.stat)?.score
            if (statValue) {
                const modifier = Math.floor((statValue - 10) / 2)
                return <li className='skillsDiv'
                    onClick={() => {
                        console.log(skill.url);
                        
                        handleItemClick('skills','', skill.url)
                    }}
                >{'♦ '+ skill.name + ' : ' + skill.score + '  (' +
                    (modifier > 0 ? '+' : '') + modifier + ') = ' + (skill.score + modifier)}</li>;
            }
        }
    });
    return (
        <InfoCard sx={{ width: '100%', maxWidth: 360 }}>
            {/* <List > */}
            <ListContainer>
...
export default createSkillsCard;